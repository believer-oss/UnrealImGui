...
void Client::CaptureImguiInput()
{
	...
	// BV BEGIN [rdunnington] - We don't draw clipped to a specific imgui window
	...
	if( ImGui::IsMousePosValid(&io.MousePos)){
		// BV BEGIN [rdunnington] - Mouse coordinates start at upper left of viewport instead of the current window region
		// 							since we don't draw clipped to a specific window
	...
	// BV BEGIN [rdunnington] - Use whole viewport instead of the current window region, we don't draw clipped to a specific window
	...
	pNewInput->mScreenSize[0]		= static_cast<uint16_t>(viewport->WorkSize.x);
	pNewInput->mScreenSize[1]		= static_cast<uint16_t>(viewport->WorkSize.y);
	// BV END
	...
	// BV BEGIN [rdunnington] - Ensure the compression skip flag is preserved if we capture input before we had a chance to send
	// pNewInput->mCompressionSkip		= mbCompressionSkipOncePending;
	pNewInput->mCompressionSkip		= mbCompressionSkipOncePending || pNewInput->mCompressionSkip;
	// BV END
	pNewInput->mFontDPIScaling		= config.mDPIScaleEnabled ? NetImguiServer::UI::GetFontDPIScale() : 1.f;
	mbCompressionSkipOncePending	= false;

	// BV BEGIN [rdunnington] - We don't draw clipped to a specific window, don't need a focused window
	// if( ImGui::IsWindowFocused() )
	// BV END
	{
